...
const selectedGrade = ref<string | null>();
...
// const dropdownMajors = ref(' ');
interface dropdownMajor {
  major: string;
}
const dropdownMajors = ref<dropdownMajor[]>([]);

const fetchMajors = async (item) => {
  try {
    const majorResult = await LXRgetmajor(item);

    if (majorResult.code === 200 && majorResult.data.length > 0) {
      dropdownMajors.value = majorResult.data;
    } else if (majorResult.data.length === 0) {
      // uniqueMajors.push({major: '数据为空'});
